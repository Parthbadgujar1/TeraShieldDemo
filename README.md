...
### Running from a downloaded ZIP on another computer

Everything the site needs is in the repository — the generated data is committed in `frontend/public/data`, so **you do not need Python, the data pipeline, a database or any API key** just to run the portal.

1. **Install Node.js 20 or newer** (LTS) from <https://nodejs.org>. Check with `node -v` and `npm -v`.
2. **Unzip** the download (GitHub → Code → Download ZIP) to a folder whose path has no special characters, e.g. `C:\TeraShield`.
3. Open a terminal **in the `frontend` folder** and run:
   ```bash
   npm ci               # exact versions from package-lock.json (use `npm install` if `npm ci` complains)
   npm run dev          # opens on http://localhost:5173
   ```
4. Sign in with `sih / sih2026` (admin) or `rescue / rescue2026` (emergency team).
5. For a production build: `npm run build`, then `npm run preview` (serves `frontend/dist`).

**Needs internet at run time (not at install time):** the live layers call public services from your browser — Open-Meteo (forecast, GloFAS), Overpass (OpenStreetMap counts and villages), OSRM (road routes), and Esri / OpenStreetMap map tiles. Without a connection the national map, the pilot, the Action Plan, Validation and Data Lab still work from the bundled data, and the 72 h overlay can use a **replay** of a real event (choose it under *Data source*). Public services can be slow or rate-limited; the app falls back to cached data and says so.

**Troubleshooting**
- *Blank page or old data after an update:* hard-refresh (Ctrl+F5). Data files are versioned per build, but a browser can hold an old page.
- *Port 5173 already in use:* stop the other process or run `npm run dev -- --port 5174`.
- *`npm ci` fails on an old Node:* upgrade to Node 20+.
- *Register / uploads / sign-in vanished:* they live in the browser's localStorage for that address (`localhost:5173` is separate from the Vercel site). Export the register as JSON before clearing browser data.

**What is deliberately not in the repository** (so a ZIP does not contain it)
- `data-pipeline/.cache/` (~300 MB of raw downloads: terrain tiles, WorldPop GeoTIFF, OSM extracts, climate cells) — it is regenerated by the pipeline, see below. You only need it if you want to rebuild the data.
- `node_modules/`, `frontend/dist/` — recreated by `npm ci` / `npm run build`.
- Your Vercel account and project settings, and GitHub repository settings (see *Deploy to Vercel*).
- Browser-side state (Zone Register entries, uploaded habitations, cached forecasts).

**Rebuilding the data on a new machine** needs Python 3.10+ and roughly 1–3 hours plus about 1 GB of disk, mostly waiting on public servers; run the commands under *Rebuild the dataset* in order. The Overpass step can fail when the public service is busy — re-run it and it resumes from the cache.
...
[`.github/workflows/refresh-alerts.yml`](.github/workflows/refresh-alerts.yml) snapshots the NDMA feed every six hours and pushes `alerts.json`, which triggers a fresh deployment. It needs **Settings → Actions → General → Workflow permissions → Read and write**. Because the bot pushes to `main`, run `git pull --rebase` before you push your own changes.

If you fork or move the repository, these settings live in the accounts, not in the code, and must be redone: the Vercel project (Root Directory `frontend`), the GitHub Actions write permission, and — if you rename the repo — the Vercel Git connection.
