...
import {
  CAPACITY_ASSUMPTIONS, HARD_REJECT, PHASES, PHASE_WEIGHTS, buildCandidates, effectiveMinutes, movementPlan, rank, ringPoints,
  type Candidate, type Phase, type PlanOption, type Weights,
} from "../../lib/relocation";
import { HAZARDS, HAZARD_BY_KEY, TIER_ACTION, TIER_COLOR, TIER_LABEL, ZONE_COLOR, ZONE_LABEL, compact, lakh } from "../../lib/risk";
...
const isSafe = (c: Candidate) => c.safety >= 0.5 && c.reject.length === 0;
...
export default function EvacuationPlanner({ scope, embedded = false }: { scope: string; embedded?: boolean }) {
  ...
  async function analyse() {
    ...
    try {
      ...
      step(0, `${ZONE_LABEL[d.zone]} district, ${d.risk.toFixed(0)}% annual multi-hazard${live ? ` · 72 h alert ${live.risk.toFixed(0)}%` : ""}`);
      ...
      step(4, `${safe.length} pass the local hazard screen (hard reject on high susceptibility; safety ≥ 50%) · ${cands.filter((c) => c.reject.length).length} hard-rejected${water ? "" : " · rivers still loading, district value used"}`);

      setBusy("Routing on the real road network…");
      const prelim = rank(cands, new Map(), { weights: norm(weights), blocked: new Set(), ttiHours: tti, prepMin: prep }).filter((o) => isSafe(o.candidate)).slice(0, 6);
      ...
      step(6, amen ? "Shelter capacity counted from mapped schools, colleges and halls within 5 km (hospitals are never counted as shelter)" : "Facilities still loading — capacity uses land parcel only, upgrades when they arrive");
  ...
  const isSafeC = isSafe;
  const mapCands: MapCandidate[] = run
    ? run.cands.map((c) => {
      const idx = options.findIndex((o) => o.candidate.id === c.id);
      return { id: c.id, lat: c.lat, lon: c.lon, safe: isSafeC(c), rank: idx >= 0 && idx < 6 ? idx + 1 : null, score: idx >= 0 ? options[idx].total : null, label: nameOf(c) };
    })
    : [];
  ...
  return (
    <div className={embedded ? "" : "page"}>
      <div className="page-head" style={embedded ? { paddingTop: 0 } : undefined}>
        <div className="grow">
          <h2 style={{ margin: 0 }}>{emergency ? "Evacuation Planner" : "Emergency evacuation"}</h2>
          <p>
            <b>Evacuation is not relocation.</b> This track moves people out of harm's way in hours or days — buses, waves, boarding priority and a
            time-to-impact clock — to a temporary safe place. Permanent, planned resettlement (land, livelihood, approvals, months to years) has its own track.
          </p>
        </div>
        <ZoneBadge zone={d.zone} label={`${d.n}: ${ZONE_LABEL[d.zone].toLowerCase()} tier`} />
      </div>
          ...
          {sel && plan && (
            <div className="card" style={{ borderTop: `4px solid ${rerouted ? "var(--orange)" : "var(--good)"}` }}>
              ...
              <div className="card-pad stack" style={{ gap: 14 }}>
                ...
                {sel.rejected && <div className="notice err small"><b>Hard-rejected site:</b> {sel.candidate.reject.join("; ")}. Shown for transparency only — it is never recommended.</div>}
                ...
                <div className="grid g2">
                  ...
                  <div className="stack" style={{ gap: 8 }}>
                    ...
                    <div className={`notice ${capOk ? "info" : "warn"} small`}>
                      {capOk ? "✓ " : "⚠ "}
                      Capacity = min(space, water): shelter {lakh(sel.candidate.capacity.shelter)} ({sel.candidate.capacity.schools} schools, {sel.candidate.capacity.halls} halls; no hospitals) + parcel {lakh(sel.candidate.capacity.land)} → <b>{lakh(sel.candidate.capacity.total)}</b> vs {lakh(pop)} people. Water {sel.candidate.capacity.water}{sel.candidate.capacity.bind === "water" ? " — binding constraint" : ""}; sanitation UNVALIDATED.
                    </div>
                ...
                <div className="grid g2">
                  <div className="stack" style={{ gap: 6 }}>
                    ...
                    <dl className="kv">
                      ...
                      <dt>Under 30 (Census age band; 0–6 needs PCA)</dt><dd>{lakh(plan.young)}</dd>
                    </dl>
                  </div>
                  <div className="stack" style={{ gap: 6 }}>
          ...
          {options.length > 0 && (
            <div className="card">
              ...
              <div className="card-pad tiny muted">
                Pareto = no other site is at least as good on safety, route, capacity and services and better on one. {run && `${run.cands.filter((c) => c.reject.length).length} of ${run.cands.length} sampled sites were hard-rejected for high local susceptibility; ${run.cands.filter((c) => !c.reject.length && c.safety < 0.5).length} more fell below the safety threshold.`}
              </div>
            </div>
          )}
        ...
        <aside className="stack info-panel" aria-label="Planner controls" style={{ top: 62 }}>
          ...
          <div className="card card-pad stack" style={{ gap: 10 }}>
            ...
            <p className="tiny muted">
              Samples 48 sites around {origin.name}, hard-rejects those with high local flood or landslide susceptibility, then routes the best six on the real road network and scores each road against forecast rain and terrain.
            </p>
            {run && (
            ...
            <Provenance title="Assumptions & limits">
              <ul className="plain">
                <li>Shelter capacity: school {CAPACITY_ASSUMPTIONS.school}, college {CAPACITY_ASSUMPTIONS.college}, hall {CAPACITY_ASSUMPTIONS.hall} people within 5 km (hospitals are never shelter); planned parcel {CAPACITY_ASSUMPTIONS.parcelHa} ha × {CAPACITY_ASSUMPTIONS.peoplePerHa} people/ha × {(CAPACITY_ASSUMPTIONS.netShare * 100).toFixed(0)}% net, scaled by flatness; then capped by water (a river within 2 km = partial proxy, else ×0.6). Sanitation is unvalidated.</li>
                <li>Hard rejects: local landslide ≥ {HARD_REJECT.landslide * 100}, flood ≥ {HARD_REJECT.flood * 100}, slope ≥ {HARD_REJECT.slope}°, or a river bank ≤ {HARD_REJECT.handM} m above low ground. High susceptibility is a veto, not a weight.</li>
